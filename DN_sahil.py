...
class dynamicDelayToggle():
    ...
    def click( self, event ):
        ...
        if self.duration < 0.5:
            ...
            ax_slider.clear() 
            sliders[self.dynDel_index].__init__(ax_slider, "Static Inh. Delay", minDelay, max_synapticDelay, gabaOnset*1000)
            sliders[self.dynDel_index].on_changed( setGabaOnset )
            dynamicDelay_toggle = 0
            ...
            ax_slider = sliders[self.dynDel_index].ax
            ax_slider.clear() 
            sliders[self.dynDel_index].__init__(ax_slider, "Dynamic Inh. Delay", 1, 10.0, 5.0)
...
def updateDisplay():
    ...
    #e_g_peak.set_x(np.argmax(exc_g))
    #i_g_onset.set_x(np.argmax(exc_g))
    ...
    gabaonset_list.append((gabaOnset- gluOnset)*1000)
...
def makeDisplay():
    ...
    img = mpimg.imread( 'Image_simulation_2.png' )
    fig = plt.figure( figsize=(10,12) )
    gridspec.GridSpec(3,3)

    #png = fig.add_subplot(311)
    png = plt.subplot2grid((3,3), (0,0), colspan=1, rowspan=1)
    imgplot = plt.imshow( img )
    plt.axis('off')

    t = np.arange( 0.0, runtime + elecPlotDt / 2.0, elecPlotDt ) * 1000 #ms

    ei_g_ax = plt.subplot2grid((3,3), (0,1), colspan=1, rowspan=1)
    simpleaxis(ei_g_ax)
    plt.ylabel( '$g$ (nS)' )
    plt.xlabel( 'Time (ms)' )
    plt.ylim(0,1.5)
    ...
    #i_g_onset, = ei_g_ax.vlines( 0., 0.,1.5, linestyle='--', alpha=0.3,color='orange')


    t = np.arange( 0.0, runtime + elecPlotDt / 2.0, elecPlotDt ) * 1000 #ms

    ei_ax = plt.subplot2grid((3,3), (0,2), colspan=1, rowspan=1)
    simpleaxis(ei_ax)
    plt.ylabel( '$I$ (pA)' )
    plt.xlabel( 'Time (ms)' )
    plt.ylim(-20,20)
    ...
    ax1 = plt.subplot2grid((3,3), (1,0), colspan=1, rowspan=1)
    simpleaxis(ax1)
    ...
    simpleaxis(ax2)
    plt.ylabel( 'Observed $V_m$ (mV)' )
    #plt.ylim( -80, -30 )
    plt.xlabel( 'Expected $V_m$ (mV)' )
    #plt.title( "SDN" )
    ...
    sdn_plot = ax2.scatter([], [], s=12, c=[], cmap=cmap,  vmin=0., vmax=6)
    last_point, = ax2.plot(last_pk[0], last_pk[1], 'o', markersize=7, markerfacecolor='k')
    sdn_cbar = plt.colorbar(sdn_plot, ax=ax2, shrink=0.8, label="I/E ratio")
    ...
    plt.axis('off')
    axcolor = 'palegreen'
    axStim = plt.axes( [0.02,0.005, 0.10,0.03], facecolor='green' )
    axKA = plt.axes( [0.14,0.005, 0.10,0.03], facecolor='green' )
    axDynDel = plt.axes( [0.26,0.005, 0.15,0.03], facecolor='green' )
    axReset = plt.axes( [0.43,0.005, 0.20,0.03], facecolor='blue' )
    axQuit = plt.axes( [0.65,0.005, 0.30,0.03], facecolor='blue' )

    for x in np.arange( 0.05, 0.21, 0.05 ):
        axes.append( plt.axes( [0.25, x, 0.65, 0.03], facecolor=axcolor ) )

    sliders.append( Slider( axes[3], "gluGbar (Mho/m^2)", 0.001, max_exc_cond, valinit = gluGbar) )
    sliders[-1].on_changed( setGluGbar )
    sliders.append( Slider( axes[1], "IE_ratio", 0.0, 6., valinit = IE_ratio) )
    ...
    sliders.append( Slider( axes[2], "Dynamic Inh. Delay", 1, 20.0, valinit = dynamicDelay, valfmt='%0.2f'))
    ...
    def reInitialize():
        global sdn_x, sdn_y, gluGbar_list, gabaonset_list
        sdn_x = []
        sdn_y = []
        IE_ratio_arr=[]
        gluGbar_list, gabaonset_list, peaktime_list = [], [], []
