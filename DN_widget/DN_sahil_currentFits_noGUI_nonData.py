...
def EI_input():
    global sdn_x, sdn_y, sdn_y2, IE_ratio_arr, EI_pair, EI_onset, spikeFlag, threshold
    dirname = sys.argv[1] 
    ...
    for coord in EI_pair.keys():
        for j, synTuple in enumerate(EI_pair[coord]):
            ...
            updateDisplay()
    if spikeFlag == 'spikes':
        with open('SDN/sdn_{}_{}_{}_{}.pkl'.format(n.date, n.index, abs(threshold), spikeFlag), 'w') as f:  # Python 3: open(..., 'wb')
            pickle.dump([sdn_x, sdn_y, sdn_y2, IE_ratio_arr, EI_pair, EI_onset], f)
    else:
        with open('SDN/sdn_{}_{}_{}.pkl'.format(n.date, n.index, spikeFlag), 'w') as f:  # Python 3: open(..., 'wb')
            pickle.dump([sdn_x, sdn_y, sdn_y2, IE_ratio_arr, EI_pair, EI_onset], f)
...
class fileDialog():
    ...
    def click( self, event ):
        global sdn_x, sdn_y, sdn_y2, IE_ratio_arr, EI_pair, EI_onset, threshold 
        dirname = get_dir()
        cellIndex = dirname.split('/')[-1]
        print("cell index is ".format(cellIndex))
        filename = dirname + '/plots/' + cellIndex + '.pkl'
        n = Neuron.load(filename)
        print("Loaded {} {}".format(n.date, n.index))
        EI_pair, EI_onset = get_EI_pairs(n)
...
def updateDisplay():
    ...
    if not np.isnan(gluOnsetIndex): 
        ...
        if spikeFlag == 'spikes':
            spikeTime = (int(gluOnset/elecPlotDt) + findFirstSpikeTime(tabvec_filtered, threshold=threshold))*elecPlotDt*1e3
            last_pk[1] = spikeTime
        else:
            last_pk[1] = pk 
    ...
    #tplot, = ax.plot( t, np.zeros(len(t)), '-', color='k' )
    #tplot_noinh, = ax.plot( t, np.zeros(len(t)), '--',color='r' )
    ...
    if spikeFlag == 'spikes':
        spikeTime_exc = findFirstSpikeTime(tabvec, threshold=threshold)*elecPlotDt*1e3
        last_pk[0] = spikeTime_exc
    else:
         last_pk[0] = pk 

    #print(spikeTime_exc, threshold)
    ...
    mainAx.scatter(pk[0], pk[1])

    #simpleaxis(ax) 
    #plt.savefig('spiking_neurons/spikes_{}.svg'.format(max(exc_g*1e9)))
    #plt.close(fig)

    #print("Done figure spiking_neurons/spikes_{}.svg".format(max(exc_g*1e9)))
    #print(frameNum)
    if makeMovie:
...
def makeDisplay():
    ...
    tplot, = ax1.plot( t, np.zeros(len(t)), '-', color='k' )
    tplot_noinh, = ax1.plot( t, np.zeros(len(t)), '--',color='r' )
